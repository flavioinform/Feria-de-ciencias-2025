...
function Registro() {
  ...
  const { login } = useContext(AppContext);   // 👈 tomar login del contexto
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error || !usuario) {
        setMensaje("RUT no está registrado");
        setTipo("error");
        return;
      }
      ...
      // 👉 guardar admin en el contexto (y localStorage, según tu AppProvider)
      const userData = {
        id: usuario.id,
      ...
      navigate("/admin/registroLogin");
    } catch (err) {
      console.error("excepcion en administrador", err);
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={cargando}
              className="w-full bg-black text-white font-semibold py-3 px-6 rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-300 transition-all duration-300 shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {cargando ? (
